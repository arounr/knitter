  ...
  return (
    <main className="flex flex-col items-center p-8 font-sans text-[var(--color-text-primary)]">
      <section className="w-full max-w-sm text-center mb-4">
        <h1 className="text-3xl font-semibold text-[var(--color-text-primary)]">Register</h1>
        <p className="text-[var(--color-text-secondary)] text-sm mt-2">Create an account to join the community.</p>
      </section>
      ...
      <section className="flex flex-col items-center w-full max-w-sm p-8 space-y-6 bg-[var(--color-card-bg)] rounded-lg shadow-lg sm:p-12">
        <Form action={handleSubmit} className="w-full space-y-4 flex flex-col items-center">
          ...
          <div className="w-full">
            <label className="block text-sm font-medium text-[var(--color-text-secondary)]" htmlFor="confirmPassword">
              Confirm Password
            </label>
            <input
              type="password"
        ...
        <Link href="/login" passHref>
          <button className="text-sm text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]">Already have an account? Login</button>
        </Link>
      </section>
    </main>
  );
